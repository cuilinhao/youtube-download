...
import { CheckIcon, DownloadIcon, LoaderIcon } from './Icons';

const options: DownloadOption[] = [
  { format: 'mp4', quality: '1080p', size: 'Best', badge: 'Best' },
  { format: 'mp4', quality: '720p', size: 'Balanced' },
  { format: 'mp3', quality: '320kbps', size: 'Audio', badge: 'Audio' }
];
...
const DownloadPanel: React.FC<DownloadPanelProps> = ({ videoUrl }) => {
  ...
  const startDownload = (idx: number, opt: DownloadOption) => {
    if (!videoUrl || downloading !== null) return;
    setDownloading(idx);

    // Kick off download in the background while we animate progress.
    const link = document.createElement('a');
    link.href = `/api/download?url=${encodeURIComponent(videoUrl)}&quality=${opt.quality}&format=${opt.format}`;
    link.download = '';
    link.target = '_blank';
    link.rel = 'noopener';
  ...
  return (
    <div className="bg-yt-dark rounded-xl border border-gray-800 p-6 flex flex-col h-full">
      ...
      <div className="space-y-4 flex-grow">
        {options.map((opt, idx) => {
          const disabled = downloading !== null || !videoUrl;
          return (
            <div
              key={idx}
              ...
              <div className="relative z-10 flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="text-xs text-gray-500">
                    {opt.size} • {opt.format === 'mp3' ? 'Audio' : 'Video'}
                  </div>
                </div>
              </div>

              <button
                onClick={() => startDownload(idx, opt)}
                disabled={disabled}
                className={`relative z-10 px-4 py-2 rounded-lg font-medium text-sm transition-all flex items-center gap-2
                ${completed === idx ? 'bg-green-500/10 text-green-500 border border-green-500/20' : 'bg-white text-black hover:bg-gray-200'}
                ${disabled && completed !== idx ? 'opacity-50 cursor-not-allowed' : ''}
              `}
              >
                {downloading === idx ? (
                  <>
                  ...
                  <>
                    ...
                    <span>{videoUrl ? 'Download' : 'Paste URL first'}</span>
                  </>
                )}
              </button>
            </div>
      ...
      <div className="mt-6 pt-4 border-t border-gray-800 text-xs text-gray-500 text-center">
        ...
        <p className="mt-1 opacity-60">Downloads are served by the built-in ytdl-core API route.</p>
      </div>
    </div>
  );
};
